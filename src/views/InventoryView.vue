    ...
    <template>
        <div id="filters">
            <div class="f">
                <button :class="{ 'filter_active': filters_activated, 'btn-filters': !filters_activated }" @click="showFilters" type="button">
                    Seleccionar filtros
                ...
                <div v-show="toggleFilters" class="list-filters">
                    <ul>
                        <li>
                            Estado
                            <select class="select-filters" name="" id="stateItem" v-model="state">
                        ...
                        <li>
                            Área
                            <select class="select-filters" name="" id="areaItem" v-model="area_id">
                                <option value="0" disabled selected>Seleccionar área</option>
                                <option @click="removeFilter()" value="0">Todas las área</option>
                            <option @click="filter()" v-for="area in areas" :key="area.id" :value="area.id">
                                {{ area.area_name }}
                            </option>
                            </select>
                        </li>
                ...
                <input class="inputGoTo" type="number" @change="changeRows()"  @input="watcherRows()" v-model="rows"  min="1" :max="items.length ">
                <span style="font-weight: bold;">&nbsp;  de {{ items.length }}</span>
            </div>
            
            <div class="b">
                <input type="search" v-model="searchByBar" name="serach" id="search" placeholder="Buscar por descripción, marca...">
            </div>
        </div>
        <div class="table-conteiner">
            ...
            <div class="sb">
                <div class="div-exports">
                    ...
                    <div @click="exportExcel" class="excel">
                        <img class="icon-exportar" src="@/assets/icon_excel.png" alt="Boton para descargar archivo excel">
                        <strong></strong>
                    </div>
                    <div @click="exportPDF" class="pdf">
                        <img  class="icon-exportar" src="@/assets/icon_pdf.png" alt="Boton para descargar archivo pdf">
                        <strong></strong>
                    </div>
                </div>

                <div class="div-btn">
                    <button :class="{ 'mrg-r': selectedItems.length === 0, 'btn-add': selectedItems.length > 0}"  @click="openModalAlta" type="button">Añadir</button>
                    <button v-if="selectedItems.length > 0" class="btn-del" @click="openModalDelete" type="button">Eliminar</button>
                </div>
            ...
            <div class="t">
                ...
                <tbody>
                    ...
                    <td @click="openModalImages(item.image)" class="scale-image"><img :src="'https://api.inbeartory.site/storage/images/' + item.image" alt="item.image" class="item-image" /></td>
                    ...
                    <td class="d-none">
                        <span style="font-size: 1rem;" v-if="item.state === 1">Activo</span>
                        <span style="font-size: 1rem;" v-else>Baja</span>
                    </td>
                    <td>
                        <img  @click="openModalEdit(item, index)" class="icon-actions"  src="@/assets/lapiz_editar.png" alt="Boton para editar">    
            ...
            <div class="pages">
                <button class="btn-pages ml" @click="backPage"><img class="icon-arrow-p" src="@/assets/arrow-l-page.png" alt="Flecha Izquierda"></button>
                <button class="num-pages" v-if="isPageStart" @click="selectPage(totalPages[1])"> <img class="arrow-2" src="@/assets/rebobinar-angulos-de-flechas-dobles.png" alt="Ultima pagina"> </button>
    
                <input type="text" readonly class="actual-page" v-model="page" >

                <button class="num-pages" v-if="isPageEnd" @click="selectPage(totalPages[totalPages.length - 1])"> <img class="arrow-2" src="@/assets/flechas-dobles-hacia-la-derecha-de-avance-rapido.png" alt="Ultima pagina"> </button>
                <button class="btn-pages" @click="nextPage"><img class="icon-arrow-p" src="@/assets/arrow-r-page.png" alt="Flecha Derecha"></button>
                <label for="GoToPage" style="font-weight: bold; font-size: 1rem;">Ir a la página:</label>
                <input type="number" class="inputGoTo" v-model="goPage" min="1" :max="totalPages.length-1">
            </div>
        </div>

        <ModalAltaItem @itemAdded="addItemToList" ref="modalAlta"/>
        <ModalEditItem @itemEdited="editItemToList" @imgItemEdited="editImgItem" ref="modalEdit"/>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            updateItemList() {
                this.$axios.get('https://api.inbeartory.site/api/v1/items')
                    .then((response) => {
            ...
            searchByFilters(){
                ...
                this.$axios.get('https://api.inbeartory.site/api/v1/items_filter', {
                    params: params
                })  
                .then(response => {
                    this.total_items = 0;
            ...
            searchBar(){
                ...
                this.$axios.get('https://api.inbeartory.site/api/v1/items/' + this.searchByBar,)
                .then(response => {
            ...
            watcherPages(){
                if (this.totalPages.length > 0 && this.page > this.totalPages[this.totalPages.length - 1]) {
                    this.page = this.totalPages[this.totalPages.length - 1];
                }
            }    
        },
        mounted() {
            this.$axios.get('https://api.inbeartory.site/api/v1/areas')
            .then((response) => {
